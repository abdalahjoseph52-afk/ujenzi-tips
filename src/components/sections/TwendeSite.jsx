...
const TwendeSite = () => {
  ...
  // Data with Verified IDs
  const videoIds = ["npnSan2ckYY", "ZRU_iTxXeOs", "OlSK2iaVAVM"];
  
  // FIX: Changed 'maxresdefault' to 'hqdefault' (Guaranteed to exist)
  ...
  return (
    <>
      <section id="twende-site" className="py-24 bg-ujenzi-dark relative overflow-hidden">
        ...
        {/* Background "Construction Tape" Effect */}
        <div className="absolute top-0 left-0 w-full h-2 bg-repeating-linear-gradient(45deg, #F59E0B, #F59E0B 10px, #0F172A 10px, #0F172A 20px) opacity-20"></div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Featured Video (The "Hero" Episode) */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            onClick={() => setSelectedVideo("npnSan2ckYY")} 
            className="relative w-full h-[500px] rounded-sm overflow-hidden border border-white/10 mb-12 group cursor-pointer"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {t.twende.list.map((video, index) => (
              <motion.div
                ...
                className="group bg-ujenzi-card border border-white/5 hover:border-ujenzi-accent/30 transition-colors duration-300 cursor-pointer"
              >
                {/* Thumbnail */}
                <div className="relative h-48 overflow-hidden">
                  <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-2 py-1 text-[10px] font-bold text-white uppercase z-10">
                    {video.category}
                  </div>
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    <h4 className="text-lg font-bold text-white leading-tight group-hover:text-ujenzi-accent transition-colors">
                      {video.title}
                    </h4>
                  </div>
                  <p className="text-slate-400 text-sm line-clamp-2 mb-4 border-l-2 border-white/10 pl-3">
                    {video.desc}
                  </p>
